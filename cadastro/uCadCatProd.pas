...
uses
  ...
  FireDAC.Comp.Client, udmConexao, uTelaHeranca, uPesquisaCatProduto;
...
procedure TfrmCadCatProd.btConsultarClick(Sender: TObject);
var
  codigo: integer;
begin
  inherited;

  frmPesquisaCatProduto:=TfrmPesquisaCatProduto.create(nil);

  try
    codigo := frmPesquisaCatProduto.SelecionarCliente;

    if codigo > 0 then
      if qryListagem.Locate('codigo', codigo, []) then
        AtualizaCampos;
  finally
    frmPesquisaCatProduto.Free;
  end;
...
procedure TfrmCadCatProd.btExcluirClick(Sender: TObject);
var
  Qry: TFDQuery;
begin
  ...
  begin
    ...
    begin
      Qry.Close;
      Qry.SQL.Clear;
      Qry.SQL.Add('DELETE FROM cat_produto WHERE codigo = '+ edCodigo.Text +'');

      Qry.ExecSQL;

    if Qry.RowsAffected > 0 then
      ShowMessage('Categoria de Produto com sucesso!')
    else
      ShowMessage('Erro ao deletar Categoria de Produto.');
...
begin
  ...
  try
    ...
    Qry.Close;
    Qry.SQL.Clear;
    Qry.SQL.Add('UPDATE cat_produto SET nome = :nome where codigo = :codigo ');

    Qry.ParamByName('nome').AsString := edDescricao.Text;
